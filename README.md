...
#### 添加名为 `CRON_HOURS` 的Variables变量 `Settings-->Secrets and variables-->Actions-->New repository variables` 注意不是Secret

-
快捷跳转地址 [https://github.com/${你的github用户名}/mimotion/settings/variables/actions](../../settings/variables/actions)
    - 填写自动执行的小时，使用UTC时间和逗号分隔，例如设置 `0,2,4,6,8,14`，会在北京时间 `8,10,12,14,16,22` 点执行
    - 未配置时默认使用 `10,12,14`，即北京时间 `18,20,22` 点执行
    - 工作流固定在每小时第17分钟触发，然后根据定时事件的创建时间判断UTC小时是否在 `CRON_HOURS` 中；即使Runner排队延迟，也不会误判执行小时
    - 小时不匹配时会立即结束，不安装依赖
    - `CRON_HOURS` 仅接受0到23的整数，配置错误会在Actions日志中明确报错
    - 手动触发不受 `CRON_HOURS` 限制

- 不要通过脚本或其他工作流反复修改 **.github/workflows/run.yml** 中的cron表达式，否则GitHub可能无法及时重新注册定时任务。
...
1. 定时工作流固定在每小时第17分钟触发，并根据 `CRON_HOURS` 判断是否执行，因此不会再通过提交代码随机修改执行分钟。
...
- 如果定时任务被跳过，请查看 `Check configured execution hour` 步骤，其中会输出事件创建时间、配置的小时和判断结果。
